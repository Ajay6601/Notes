# GraphRAG, Knowledge Graphs & RAG Variants - Interview-Ready Guide

**🎯 Purpose:** Master RAG concepts to crack any AI/ML interview
...
## 🎯 The Big Picture: Why RAG Matters {#big-picture}
...
- LLM: ❌ "I don't have information about future events"
- RAG: ✅ Retrieves recent news articles → Answers correctly

**2. Hallucination**
- LLM generates plausible but **false** information
- Example: "Company X's Q3 revenue was $50M" (made up)
- RAG: ✅ Retrieves actual financial report → Cites source
...
- RAG: ✅ Retrieves from company knowledge base

**4. Cost of Retraining**
- Retraining GPT-4 costs millions of dollars
- Takes weeks/months
- RAG: ✅ Just update document database (costs pennies)
...
- Split by paragraph → sentence → character
- Preserves hierarchy
...
**Key Considerations:**
- **Dimension size:** Higher ≠ always better (diminishing returns, cost)
...
4. **Hierarchy**: Subclasses (CEO ⊂ Person)
...
```
Documents → LLM Extraction → Knowledge Graph → Community Detection → Summarization → Query
```
...
│   ├── Child Chunk 1a (200 tokens) ← Embedded for search
│   ├── Child Chunk 1b (200 tokens) ← Embedded for search
│   └── Child Chunk 1c (200 tokens) ← Embedded for search
...
│
├─ Yes, AWS-native? → Amazon Neptune
├─ Yes, Azure-native? → Azure Cosmos DB (Gremlin)
├─ Yes, analytics-heavy? → TigerGraph
├─ Yes, general purpose? → Neo4j
│
└─ No, just vector search? → Qdrant, Pinecone, Weaviate
```
...
│
│   ⚠️ Trade-off: Slower queries (500-2000ms)
│   💰 Trade-off: Higher cost (more LLM calls)
│
Low Latency (Vector RAG)
    ⚠️ Trade-off: Lower accuracy on complex queries
    💰 Benefit: Lower cost
```
...
High Setup (GraphRAG)
│
│   ✅ Benefit: Better long-term performance
│   ⚠️ Trade-off: Weeks of setup (KG building, tuning)
│
Low Setup (Vector RAG)
    ✅ Benefit: Deploy in days
    ⚠️ Trade-off: May need frequent tuning as queries evolve
```
...
│
│   ✅ Can show: Entity → Relation → Entity paths
│   ⚠️ Trade-off: Complex pipeline, more to debug
│
Low Explainability (Vector RAG)
    ✅ Simple pipeline
    ⚠️ Hard to explain why a chunk was retrieved
```
...
```
Budget?
├─ Tight → ChromaDB (dev) or pgvector (prod)
├─ Medium → Qdrant or Weaviate (self-hosted)
└─ High → Pinecone (managed)

Scale?
├─ <1M vectors → pgvector, ChromaDB
...
3. Reranking (50-200ms)
   - Cross-encoder on top 100 → top 10 (100ms)
...
**Answer:**
1. Skip reranking → Lose 5-10% accuracy, save 100ms
2. Use smaller embedding model → Lose 2-3% accuracy, save 5ms
3. Reduce candidates (100 → 50) → Lose 3% recall, save 15ms
4. **Use GPT-4o-mini instead of GPT-4o** → Save 1000ms (biggest win)
...
- **Structure:** Interconnected (people ↔ projects ↔ teams)
- **Queries (likely):** 
...
2. **Entity-centric:** People, projects, teams are entities → natural for KG
3. **Explainability:** Can show relationship paths (Alice → Project X → Bob)
...
**Vector Database:**
```
Structure: Document → Embedding (vector)
Example: "Paris is the capital of France" → [0.1, 0.3, ..., 0.8]
...
def kg_enhanced_retrieval(query):
    ...
    # Paris → {France, Eiffel Tower, Seine River}
...
for doc in sample_docs:
    ...
    # Metrics
    precision = len(extracted ∩ gold) / len(extracted)
    recall = len(extracted ∩ gold) / len(gold)
...
for doc in sample_docs:
    ...
    # Exact match
    exact_precision = len(extracted ∩ gold) / len(extracted)
...
- **Negation blindness:** "No evidence of diabetes" → Incorrectly extracts "diabetes"
  - **Fix:** Negation detection in prompt

**2. Relationship Extraction Failures**
- **Implicit relationships:** "Aspirin reduces pain" → (Aspirin, TREATS, Pain)?
...
**4. Scale Issues**
- **Extraction cost:** 10,000 docs × $0.01/doc = $100 (GPT-4)
...
- **Self-RAG:** 
  ...
  - Confidence: High → Skip retrieval
...
```
1000 queries/day
- 400 general (skip retrieval): 400 × $0.006 = $2.40
- 600 specific (full RAG): 600 × $0.076 = $45.60
Total: $48/day = $1,440/month

Standard RAG: 1000 × $0.06 = $60/day = $1,800/month
...
# Build relationships
# Co-mention: If "Project X" and "Team Y" appear in same meeting → edge
...
BM25: High score (rare term → high IDF)
...
# Typical findings:
# α=0.0 (pure BM25): Good for exact matches
# α=0.5 (balanced): Good for mixed queries
# α=0.7-0.8 (mostly vector): Best for most datasets
# α=1.0 (pure vector): Good for conceptual queries
```
...
├─ Technical terms, IDs, codes → Lower α (more BM25)
├─ Conceptual, paraphrased → Higher α (more vector)
├─ Mixed → α=0.7 (default)
└─ Multilingual → α=1.0 (vector only)

Dataset characteristics:
├─ Well-structured, consistent terminology → BM25 works well
├─ Natural language, varied phrasing → Vector better
└─ Both → Hybrid (α=0.7)
```
...
Load Balancer
    ↓
    ├─→ Query Processing Service (3 instances)
    │       ↓
    │   Embedding (text-embedding-3-large)
    │       ↓
    ├─→ Vector DB (Qdrant cluster: 3 nodes)
    │   
    ├─→ Reranker Service (Optional, 2 instances)
    │
...
**Infrastructure:**
- **Cluster:** 3 × c5.2xlarge (AWS)
  - 8 vCPU, 16 GB RAM each
  - SSD storage: 500 GB per node
- **Documents:** 100,000 documents × 3KB avg = 300 MB
- **Vectors:** 100,000 × 3072 × 4 bytes (float32) = 1.2 GB
- **Total storage:** ~2 GB (with HNSW index overhead)
- **Cost:** $400/month (3 nodes × $133/month)

**Alternative (cheaper):**
- **Managed Qdrant Cloud:** $99/month (1 node, suitable for 100K vectors)
- **Self-hosted on smaller instances:** $150/month (2 × t3.large)
...
- Context: 10 chunks × 200 tokens = 2,000
- Query: 50 tokens
...
Total tokens/month:
- Input: 300,000 × 2,050 = 615M tokens
- Output: 300,000 × 500 = 150M tokens
...
Cost:
- Input: 615 × $0.150 = $92.25
- Output: 150 × $0.600 = $90.00
- Total: $182.25/month
```
...
|-----------|-------------|-------|
| API Gateway (3 instances) | $80 | t3.medium × 3 |
...
- Cost: (1.2M × $10) + (0.3M × $30) = $21/day = $630/month

New (GPT-4o-mini):
- Input: $0.150 per 1M tokens
- Output: $0.600 per 1M tokens
- Cost: (1.2M × $0.150) + (0.3M × $0.600) = $0.36/day = $10.8/month

Savings: $630 - $11 = $619/month (41% of target)
Accuracy impact: -3 to -5% (based on benchmarks)
```

**Lever 2: Reduce context size (10 chunks → 5 chunks)**
```
Current: 10 chunks × 200 tokens = 2,000 tokens
New: 5 chunks × 200 tokens = 1,000 tokens
...
|--------|---------|----------------|------------|
| GPT-4 → GPT-4o-mini | $619/month | -3 to -5% | Low |
| 10 chunks → 5 chunks | $5/month | -2 to -4% | Low |
...
# Cost:
# 80% × $11/month (GPT-4o-mini) = $8.80
# 20% × $630/month (GPT-4) = $126
# Total: $135/month
...
```
Infrastructure:
- 1 × A100 GPU instance (AWS p4d.24xlarge) = $32/hour
...
1. ✅ GPT-4 → GPT-4o-mini: -$619/month, -5% accuracy
2. ✅ 10 chunks → 5 chunks: -$5/month, -3% accuracy
3. ✅ Aggressive caching (50%): -$3/month, -0% accuracy
4. ✅ Self-hosted vector DB: -$49/month, -0% accuracy
5. ✅ Prompt compression: -$3/month, -2% accuracy
6. ✅ Skip reranker (if using): -$0 already, -0%
...
1. ✅ GPT-4 → GPT-4o-mini: -$619, -5% accuracy (85% → 80%)
2. ✅ Caching (50%): -$3
3. ✅ Self-hosted vector DB: -$49
4. ❌ Skip aggressive prompt compression (hurts accuracy)
...
**Total Coverage:**
- ✅ GraphRAG & Knowledge Graphs explained
- ✅ Different RAG types (Vector, Hybrid, Self-RAG, CRAG, etc.)
- ✅ Graph databases comparison
- ✅ Detailed trade-off analysis
- ✅ 12 production-grade interview questions with detailed answers
- ✅ Real-world examples from Microsoft, Meta, OpenAI, etc.
